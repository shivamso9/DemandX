...
def main(Actuals, LikeItem, NPIForecast, Last13Weeks, Active, df_keys):
    ...
    plugin_name = "DP301NPIDisaggRegionToCountry"
    ...
    try:
        ...
        PlaItem = "Item.[Planning Item]"
        Item = "Item.[Stat Item]"
        Location = "Location.[Stat Location]"
        Country = "Location.[Location Country]"
        Channel = "Channel.[Stat Channel]"
        PlaChannel = "Channel.[Planning Channel]"
        Account = "Account.[Stat Account]"
        PlaAccount = "Account.[Planning Account]"
        Pnl = "PnL.[Stat PnL]"
        PlaPnl = "PnL.[Planning PnL]"
        DemDom = "Demand Domain.[Stat Demand Domain]"
        PlaDemDom = "Demand Domain.[Planning Demand Domain]"
        Region = "Region.[Stat Region]"
        PlaRegion = "Region.[Planning Region]"
        Version = "Version.[Version Name]"
        LikeItemCol = "Like Item Selected"
        ActualCol = "Actual"
        StatFcst = "TLG Stat Fcst"
        IntFcst = "TLG Internal Country Fcst"
        History = "History"
        SumHistory = "SumOfHistory"
        AvgActual = "AvgActual"
        Ratio = "Ratio"
        NPIFcst = "NPI Fcst"
        SFTI = "Stat Fcst TLG Internal"
        ...
        # Merge NPIForecast and Active => to get the active countries in a stat location, merge NPIForecast and Like Item => To get the StatItem and LikeItem in one dataframe
        Active = Active[Active["Active"]]
        Active.drop("Active", axis=1, inplace=True)
        NPIForecast = pd.merge(
            NPIForecast,
            Active,
            on=[Version, Channel, Location, Item],
            how="inner",
        )
        NPIForecast = pd.merge(
            NPIForecast,
        ...
        NPIForecast.drop(
            ["Item.[Stat Item]_actual", "Time.[Partial Week]_actual"],
            axis=1,
            inplace=True,
        )
        ...
        Actuals[AvgActual] = Actuals.groupby(
            [Version, Channel, Account, Region, Pnl, DemDom, Country, Item]
        ...
        # For each Stat Location, calculate SumHistory for each PW, then ratio: History/SumHistory. If for an Item there's no Like Item or ratio is zero: put ratio as equally divided between countries.
