...
class Constants:
    ...
    VALIDITY_STATUS = "History Realignment Status System"
...
def main(
    ...
    plugin_name = "DP047FlexibleHistoryRealignment_Sell_In_Sell_Out"
    logger.info("Executing {} for slice {} ...".format(plugin_name, df_keys))

    if Actual is None or Full_Actual is None or RealignmentRules is None or KeyFigures is None:
        logger.warning(
    ...
    if ~KeyFigures.empty:
        measures_to_consider = KeyFigures[KeyFigures[Constants.INCLUDE_IN_HISTORY_REALIGNMENT]][
            Constants.DATA_OBJECT
        ].to_list()
    ...
    try:
        ...
        # Filter RealignmentRules based on the key figures marked for inclusion.
        sell_in_flag = (
            KeyFigures[Constants.DATA_OBJECT_TYPE].str.contains(Constants.SELL_IN, na=False).any()
        )
        sell_out_flag = (
            KeyFigures[Constants.DATA_OBJECT_TYPE].str.contains(Constants.SELL_OUT, na=False).any()
        )

        if sell_in_flag:
            RealignmentRules = RealignmentRules[
                RealignmentRules[Constants.DATA_OBJECT].str.contains(Constants.SELL_IN, na=False)
            ]
        elif sell_out_flag:
            RealignmentRules = RealignmentRules[
                RealignmentRules[Constants.DATA_OBJECT].str.contains(Constants.SELL_OUT, na=False)
            ]
        else:
            logger.warning(
                "Neither 'Sell In' nor 'Sell Out' key figures are marked for realignment. "
                "No realignment rules will be applied for this slice."
            )
            return RealignmentOP, ActualOP
