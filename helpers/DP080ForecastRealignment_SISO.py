...
def append_input_output_data(
    ...
    ForecastRaw = ForecastRaw.merge(
        intersections,
        how="left",
        indicator=True,
    )
    ForecastRaw = ForecastRaw[ForecastRaw["_merge"] == "left_only"]
    ForecastRaw.drop(columns=["_merge"], inplace=True)
    ForecastRaw = pd.concat(
        [
            ForecastRaw,
...
def main(
    ...
    plugin_name = "DP080ForecastRealignment_SISO"
    ...
    actual_contribution = str(HistoryMeasure)
    ...
    # output
    forecast_dimensions = [x for x in ForecastRaw.columns if is_dimension(x)]

    output_override_measures = [
        x
        for x in ForecastRaw.columns
        if (x not in forecast_dimensions and x in override_measure_list)
    ]
    output_override_measures = [x for x in output_override_measures if x in measures_to_consider]

    output_non_override_measures = [
        x.replace(fcst_raw_measure_suffix, "")
        for x in ForecastRaw.columns
        if x not in forecast_dimensions + output_override_measures
    ]
    output_non_override_measures = [
        x for x in output_non_override_measures if x in measures_to_consider
    ]

    output_cols = forecast_dimensions + output_override_measures + output_non_override_measures

    forecast_columns_mapping = {}
    for key in output_non_override_measures:
        forecast_columns_mapping[key] = [
            value for value in ForecastRaw.columns if key + fcst_raw_measure_suffix == value
        ][0]
    for key in output_override_measures:
        forecast_columns_mapping[key] = [
            value
            for value in ForecastRaw.columns
            if (key in value and fcst_raw_measure_suffix not in value)
        ][0]
    forecast_columns_mapping = {value: key for key, value in forecast_columns_mapping.items()}

    # override raw output
    override_column_mapping = {
        key: key + fcst_raw_measure_suffix for key in output_override_measures
    }
    non_override_column_mapping = {
        key + fcst_raw_measure_suffix: key for key in output_non_override_measures
    }
    RawOverrideOutput = pd.DataFrame(
        columns=forecast_dimensions + list(override_column_mapping.values())
    )
    ...
    actual_contribution_per_output = str(ActualContriOutputMeasure)
    ...
    try:
        ...
        forecast_measures = [x for x in forecast_columns_mapping.keys()]

        forecast_override_measures = [x for x in forecast_measures if x in output_override_measures]
        forecast_non_override_measures = list(
            set(forecast_measures) - set(forecast_override_measures)
        )

        ForecastRaw.drop(
            columns=list(
                set(ForecastRaw.columns) - set(forecast_dimensions).union(set(forecast_measures))
            ),
            inplace=True,
        )

        # override raw measures output
        forecast_raw_output = ForecastRaw.copy()

        forecast_raw_output.rename(columns=override_column_mapping, inplace=True)

        forecast_raw_output = forecast_raw_output[
            forecast_dimensions + list(override_column_mapping.values())
        ]
        ...
        if (
            ...
            ForecastRaw.rename(columns=forecast_columns_mapping, inplace=True)
            RealignedForecast = ForecastRaw[output_cols]
        ...
        # flag to null data for
        flag = "Sell Out" in RealignmentRules[data_object].iloc[0]
        ...
        group_by_cols = list(
            set(ActualContribution.columns).intersection(set([version_col] + leaf_dimensions))
        )
        ActualContribution = ActualContribution.groupby(
            group_by_cols, observed=True, as_index=False
        ...
        mappings_to_process = {item.split(".")[0] for item in forecast_dimensions}
        columns_mapping = {
            key: value for key, value in columns_mapping.items() if key in mappings_to_process
        }

        planning_dimensions = [dim for dim in planning_dimensions if dim in forecast_dimensions]

        for dim, mapping in columns_mapping.items():
            ActualContribution = ActualContribution.merge(
                mapping[1][[mapping[0], mapping[4]]],
                on=mapping[4],  # Explicitly define the merge key
                how="inner",
            )
            # if (dim == "Item") or (dim == "Location"):
        ...
        Forecast_Raw_override = ForecastRaw[forecast_dimensions + forecast_override_measures]
        Forecast_Raw_non_override = ForecastRaw[
            forecast_dimensions + forecast_non_override_measures
        ]
        ...
        non_override_output_list = []
        assortment_intersections_list = []
        is_assorted_intersections_list = []
        override_intersections_already_considered_list = []
        non_override_intersections_already_considered_list = []
        override_raw_output_list = []
        for name, group in RealignmentRules.groupby(
            ...
            override_intersections_considered = concat_to_dataframe(
                override_intersections_already_considered_list
            )
            non_override_intersections_considered = concat_to_dataframe(
                non_override_intersections_already_considered_list
            )
            ...
            override_non_override_mapping = {
                "Override": (
                    forecast_columns_mapping,
                    forecast_dimensions,
                    forecast_override_measures,
                    Forecast_Raw_override,
                    override_intersections_considered,
                ),
                "Non Override": (
                    forecast_columns_mapping,
                    forecast_dimensions,
                    forecast_non_override_measures,
                    Forecast_Raw_non_override,
                    non_override_intersections_considered,
                ),
            }

            override_non_override_mapping = {
                key: value
                for key, value in override_non_override_mapping.items()
                if not value[3].empty
            }
            ...
            # if location_col in assortment_intersections.columns:
            assortment_intersections.drop(columns=[location_col], inplace=True)
            assortment_intersections.drop(columns=[item_col], inplace=True)
            ...
            common_cols = [col for col in planning_level_cols if col in forecast_dimensions]
            relevant_forecast_raw_output = forecast_raw_output.merge(
                override_raw_intersections[common_cols].drop_duplicates(),
                on=common_cols,
            )
            override_raw_output_list.append(relevant_forecast_raw_output)
            ...
            processed_dataframes = {
                ...
                for name, value in override_non_override_mapping.items()
            }

            (
                Forecast_Raw_override_output,
                intersections_already_considered_override,
            ) = get_forecast_data(
                processed_dataframes,
                "Override",
                forecast_dimensions,
            ...
            (
                Forecast_Raw_non_override_output,
                intersections_already_considered_non_override,
            ) = get_forecast_data(
                processed_dataframes,
                "Non Override",
                forecast_dimensions,
            ...
            override_output_list.append(Forecast_Raw_override_output)
            non_override_output_list.append(Forecast_Raw_non_override_output)

            Forecast_Raw_override = append_input_output_data(
                dimensions=list(set(forecast_dimensions) - set([version_col, partial_week_col])),
                columns_mapping=forecast_columns_mapping,
                ForecastRaw=Forecast_Raw_override,
                Forecast_Raw_output=Forecast_Raw_override_output,
            )
            Forecast_Raw_non_override = append_input_output_data(
                dimensions=list(set(forecast_dimensions) - set([version_col, partial_week_col])),
                columns_mapping=forecast_columns_mapping,
                ForecastRaw=Forecast_Raw_non_override,
                Forecast_Raw_output=Forecast_Raw_non_override_output,
            )

            logger.info("appending output data ...")
            override_intersections_already_considered_list.append(
                intersections_already_considered_override
            )
            non_override_intersections_already_considered_list.append(
                intersections_already_considered_non_override
            )

        RealignedForecast_override = concat_to_dataframe(override_output_list)
        RealignedForecast_non_override = concat_to_dataframe(non_override_output_list)
        ...
        relevant_forecast_raw_output = concat_to_dataframe(override_raw_output_list)

        if RealignedForecast_override.empty:
            RealignedForecast_override = pd.DataFrame(
                columns=forecast_dimensions + output_override_measures
            )
        if RealignedForecast_non_override.empty:
            RealignedForecast_non_override = pd.DataFrame(
                columns=forecast_dimensions + output_non_override_measures
            )
        if Assortment.empty:
            Assortment = pd.DataFrame(columns=assortment_output_cols)
        if IsAssorted_output.empty:
            IsAssorted_output = pd.DataFrame(columns=is_assorted_output_cols)
        if relevant_forecast_raw_output.empty:
            relevant_forecast_raw_output = pd.DataFrame(
                columns=forecast_dimensions + list(override_column_mapping.values())
            )
        ...
        RealignedForecast_override = get_remaining_intersections(
            source_df=RealignedForecast_override,
            target_df=Forecast_Raw_override,
            planning_dimensions=planning_dimensions,
            column_mapping={},
            cols_required_in_output=forecast_dimensions + output_override_measures,
        )
        RealignedForecast_non_override = get_remaining_intersections(
            source_df=RealignedForecast_non_override,
            target_df=Forecast_Raw_non_override.rename(columns=non_override_column_mapping),
            planning_dimensions=planning_dimensions,
            column_mapping=non_override_column_mapping,
            cols_required_in_output=forecast_dimensions + output_non_override_measures,
        )

        RealignedForecast = pd.merge(
            RealignedForecast_override.drop_duplicates(),
            RealignedForecast_non_override.drop_duplicates(),
            how="outer",
        )

        RealignedForecast.drop_duplicates(subset=forecast_dimensions, keep="last", inplace=True)

        RawOverrideOutput = relevant_forecast_raw_output.copy()
        if flag:
            Assortment = pd.DataFrame(columns=assortment_output_cols)
            IsAssorted = pd.DataFrame(columns=is_assorted_output_cols)
