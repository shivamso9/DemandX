...
def main(
    ...
    try:
        ...
        STAT_FCST = o9PySparkConstants.STAT_FCST
        STAT_FCST_PL_W_LAG = o9PySparkConstants.STAT_FCST_PL_W_LAG
        STAT_FCST_PL_W_LAG_ERROR = o9PySparkConstants.STAT_FCST_PL_W_LAG_ERROR
        STAT_FCST_PL_W_LAG_ABS_ERROR = o9PySparkConstants.STAT_FCST_PL_W_LAG_ABS_ERROR
        STAT_FCST_PL_W_LAG_WMAPE = o9PySparkConstants.STAT_FCST_PL_W_LAG_WMAPE
        STAT_FCST_PL_W_LAG_ACCURACY = o9PySparkConstants.STAT_FCST_PL_W_LAG_ACCURACY
        STAT_FCST_PL_W_LAG_BIAS = o9PySparkConstants.STAT_FCST_PL_W_LAG_BIAS
        STAT_FCST_PL_W_LAG_TRACKING_SIGNAL = o9PySparkConstants.STAT_FCST_PL_W_LAG_TRACKING_SIGNAL
        STAT_FCST_PL_W_LAG_TRACKING_SIGNAL_INDICATOR = (
            o9PySparkConstants.STAT_FCST_PL_W_LAG_TRACKING_SIGNAL_INDICATOR
        )

        STAT_FCST_RL_W_LAG = o9PySparkConstants.STAT_FCST_RL_W_LAG
        STAT_FCST_RL_W_LAG_ABS_ERROR = o9PySparkConstants.STAT_FCST_RL_W_LAG_ABS_ERROR
        ...
        ActualGrains = ",".join([col for col in Actual.columns if is_dimension(col)])
        ...
        for idx, (name, data) in enumerate(data_dict.items()):
            ...
            cols_to_drop = [col for col in data.columns if col.startswith("Time") and col != WEEK]
        ...
        OutputItem = Output.withColumn(
            ...
            when(
                ...
                col(W_to_PW_Lag_Association) * col(STAT_FCST),
            ),
        )
        ...
        OutputItem = OutputItem.withColumn(
            STAT_FCST_PL_W_LAG_ERROR,
            when(col(ACTUAL) < 0, col(STAT_FCST_PL_W_LAG))
            .when((col(STAT_FCST_PL_W_LAG) == 0) & (col(ACTUAL) == 0), 0)
            .when(isnull(col(STAT_FCST_PL_W_LAG)) & (col(ACTUAL) == 0), None)
            .when(isnull(col(ACTUAL)) & (col(STAT_FCST_PL_W_LAG) == 0), None)
            .when((isnull(col(ACTUAL))) & (isnull(col(STAT_FCST_PL_W_LAG))), None)
            .otherwise(coalesce(col(STAT_FCST_PL_W_LAG), lit(0)) - coalesce(col(ACTUAL), lit(0))),
        )

        # Calculate the Fcst W Lags Abs Error
        OutputItem = OutputItem.withColumn(
            STAT_FCST_PL_W_LAG_ABS_ERROR, abs(col(STAT_FCST_PL_W_LAG_ERROR))
        )

        # Calculate the stat fcst w lag mape
        OutputItem = OutputItem.withColumn(
            STAT_FCST_PL_W_LAG_WMAPE,
            when(
                ~isnull(col(STAT_FCST_PL_W_LAG_ABS_ERROR))
                & (col(STAT_FCST_PL_W_LAG_ABS_ERROR) != 0),
                when(col(ACTUAL) == 0, 1).otherwise(
                    col(STAT_FCST_PL_W_LAG_ABS_ERROR) / col(ACTUAL)
                ),
            ).otherwise(lit(None)),
        )
        ...
        OutputItem = OutputItem.withColumn(
            STAT_FCST_PL_W_LAG_ACCURACY,
            when(
                ~isnull(col(ACTUAL)),
                when((1 - col(STAT_FCST_PL_W_LAG_WMAPE)) <= 0, 0)
                .when((1 - col(STAT_FCST_PL_W_LAG_WMAPE)) >= 1, 1)
                .otherwise(1 - col(STAT_FCST_PL_W_LAG_WMAPE)),
            ),
        )

        # Calculate the stat fcst w lag bias
        OutputItem = OutputItem.withColumn(
            STAT_FCST_PL_W_LAG_BIAS,
            when(
                col(ACTUAL) != 0,
                col(STAT_FCST_PL_W_LAG_ERROR) / col(ACTUAL),
            ).otherwise(lit(None)),
        )
        ...
        OutputItem = OutputItem.withColumn(
            ...
            when(
                col(ACTUAL) != 0,
                when((col(STAT_FCST_PL_W_LAG) / col(ACTUAL)) <= 0.75, -1)
                .when((col(STAT_FCST_PL_W_LAG) / col(ACTUAL)) >= 1.25, 1)
                .otherwise(0),
            ),
        )
        ...
        OutputItem = OutputItem.withColumn(
            STAT_FCST_PL_W_LAG_TRACKING_SIGNAL,
            sum_(col(STAT_FCST_PL_W_LAG_TRACKING_SIGNAL_INDICATOR)).over(windowspec),
        )

        # check if all the grains are present for Stat Fcst
        logger.debug(f"StatFcstGrains are : {StatFcstGrains}")
        grains_not_present = [col for col in StatFcstGrains if col not in OutputItem.columns]
        ...
        OutputItem = OutputItem.withColumn(
            STAT_FCST_PL_W_LAG_TRACKING_SIGNAL,
            col(STAT_FCST_PL_W_LAG_TRACKING_SIGNAL).cast(DoubleType()),
        ).withColumn(
            STAT_FCST_PL_W_LAG_TRACKING_SIGNAL_INDICATOR,
            col(STAT_FCST_PL_W_LAG_TRACKING_SIGNAL_INDICATOR).cast(DoubleType()),
        )
        logger.debug(
        ...
        OutputL4 = OutputL4.withColumn(
            STAT_FCST_RL_W_LAG_ABS_ERROR,
            when(coalesce(col(ACTUAL), lit(0)) < 0, col(STAT_FCST_RL_W_LAG))
            .when(isnull(col(STAT_FCST_RL_W_LAG)) & (col(ACTUAL) == 0), None)
            .when(isnull(col(ACTUAL)) & (col(STAT_FCST_RL_W_LAG) == 0), None)
            .otherwise(abs(col(STAT_FCST_RL_W_LAG) + (-1 * col(ACTUAL)))),
        )
        ...
        OutputItem = Output.drop(o9PySparkConstants.FORECAST_ITERATION)
