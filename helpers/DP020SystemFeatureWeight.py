...
def calculate_weight(
    ...
    for the_col in dist_cols:
        ...
        try:
            ...
            # fields to groupby
            fields_to_groupby = [time_col, the_col, search_space]
...
def main(
    ...
    plugin_name = "DP020SystemFeatureWeight"
    ...
    like_item_search_space = "Like Item Search Space"
    ...
    # output measures
    sys_feature_weight_col = "System Feature Weight"
    ...
    try:
        ...
        if len(SearchSpace) == 0:
            logger.warning("Like Item Search Space is empty for slice : {}...".format(df_keys))
        ...
        sales = (
            sales.groupby([version_col, item_col, relevant_time_name], observed=True)
        ...
        sales = fill_missing_dates(
            actual=sales,
            forecast_level=[version_col, item_col],
        ...
        req_cols = [item_col, relevant_time_name, like_item_actual_col]
        ...
        # Join on item to get item attributes
        sales_with_item_attributes = pd.merge(sales, Item, how="left", on=item_col)

        # fill missing values with "dummy"
        object_cols = list(sales_with_item_attributes.select_dtypes(["object"]).columns)

        # convert from string to float
        if numerical_cols[0] != "":
            sales_with_item_attributes[numerical_cols] = sales_with_item_attributes[
                numerical_cols
            ].replace({"": "nan"})
            sales_with_item_attributes[numerical_cols] = sales_with_item_attributes[
                numerical_cols
            ].replace({"NaN": "nan", "NULL": "nan"})
            columns_with_non_convertible_values = [
                ...
                if any(
                    ...
                    and val.lower() != "nan"
                    for val in sales_with_item_attributes[col]
                )
            ]
            logger.info(
            ...
            sales_with_item_attributes[numerical_cols].fillna("nan", inplace=True)
            sales_with_item_attributes[numerical_cols] = sales_with_item_attributes[
                numerical_cols
        ...
        sales_with_item_attributes.replace({"": dummy_col}, inplace=True)
        sales_with_item_attributes.replace({"nan": dummy_col}, inplace=True)
        sales_with_item_attributes[object_cols].fillna(dummy_col, inplace=True)

        # check if Numerical Cols are empty
        if len(numerical_cols) != 0 and numerical_cols[0] != "":
            for quant_col in numerical_cols:
                qt = QuantileTransformer(n_quantiles=4, output_distribution="uniform")
                values = sales_with_item_attributes[[quant_col]].values
                transformed_values = qt.fit_transform(values)
                sales_with_item_attributes[quant_col] = transformed_values

                # assign categories A, B, C, D based on the transformed values
                bin_edges = [0.3, 0.5, 0.8, float("inf")]
                bin_indices = np.digitize(
                    np.array(sales_with_item_attributes[quant_col]), bin_edges
                )
                bin_labels = {
                    0: category_a,
                    1: category_b,
                ...
                sales_with_item_attributes[quant_col] = bin_indices.ravel()
                sales_with_item_attributes[quant_col] = sales_with_item_attributes[quant_col].map(
                    bin_labels
                )
        ...
        sales_with_item_attributes = sales_with_item_attributes.applymap(
        ...
        search_space = SearchSpace[like_item_search_space][0]
        exclude_level_list = [search_space]
        search_space = "Item.[" + search_space + "]"

        # get mapping between FeatureLevel and Search Space
        feature_space_mapping = sales_with_item_attributes[
            [FeatureLevel, search_space]
        ].drop_duplicates()

        sales_with_item_attributes = sales_with_item_attributes[required_cols]
        ...
        if FeatureLevel != search_space:
            ...
            mapping = out_feature_imp_df_search_level[
                [search_space, item_feature_col]
            ].drop_duplicates()
            missing_features = pd.merge(
                mapping,
                out_feature_imp_df[[search_space, item_feature_col]],
                on=[search_space, item_feature_col],
                how="left",
                indicator=True,
            )
            missing_features = missing_features[missing_features["_merge"] == "left_only"]
            new_rows = missing_features[[search_space, item_feature_col]].copy()
            new_rows[sys_feature_weight_col] = 1.1
            new_rows = pd.merge(
                new_rows,
                out_feature_imp_df[[search_space, FeatureLevel]].drop_duplicates(),
                on=search_space,
                how="left",
            )
            ...
            final_feature_imp_df.drop(
                [ratio_col, total_normal_col, search_space],
                axis=1,
                inplace=True,
            )
