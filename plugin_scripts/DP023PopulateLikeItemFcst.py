...
Input Queries:
    likeItemMappings - Select (FROM.[Item].[Planning Item] * FROM.[Account].[Planning Account] * FROM.[Channel].[Planning Channel] * FROM.[Region].[Planning Region] * FROM.[PnL].[Planning PnL]  *  FROM.[Location].[Planning Location] * TO.[Item].[Planning Item] * Version.[Version Name]) on row, ({Edge.[620 Like Item Match].[Final Like Item Selected],Edge.[620 Like Item Match].[Final Like Item Weight]}) on column where {RelationshipType.[620 Like Item Match], Edge.[620 Like Item Match].[Final Like Item Selected]==TRUE};
...
from helpers.DP023PopulateLikeItemFcst import main
