"""
    Plugin Information:
    --------------------
		Plugin : DP122GenerateStatFcstNPIBB
		Version : 0.0.0
    ...
    Input Queries:
    --------------------
        NPILevels : Select ([Version].[Version Name] * &AllNPILevel ) on row, ({Measure.[Global NPI Account Level], Measure.[Global NPI Channel Level], Measure.[Global NPI Demand Domain Level], Measure.[Global NPI Item Level], Measure.[Global NPI Location Level], Measure.[Global NPI PnL Level], Measure.[Global NPI Region Level]}) on column;
		...
		StatFcstNPIBB : Select ([Version].[Version Name].[CurrentWorkingView] * [Region].[Planning Region] * [Location].[Planning Location] * [Channel].[Planning Channel] * [PnL].[Planning PnL] * [Item].[Planning Item] * [Demand Domain].[Planning Demand Domain] * [Time].[Partial Week] * [Account].[Planning Account] ) on row, ({Measure.[Stat Fcst NPI BB]}) on column;

    Output Variables:
    --------------------

        StatFcstNPIBBbyLevel
...
from helpers.DP122GenerateStatFcstNPIBB import main
...
NPILevels = O9DataLake.get("NPILevels")
StatFcstNPIBB = O9DataLake.get("StatFcstNPIBB")
...
# Execute Main
(StatFcstNPIBBbyLevel) = main(
    # Data
    NPILevels=NPILevels,
    StatFcstNPIBB=StatFcstNPIBB,
    # Master data
...
# Save Output Data
O9DataLake.put("StatFcstNPIBBbyLevel", StatFcstNPIBBbyLevel)
