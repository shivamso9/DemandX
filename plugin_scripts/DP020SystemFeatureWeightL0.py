"""
Plugin : DP020SystemFeatureWeightL0
Version : 0.0.0
...
Input Queries:
    ...
    SearchParameters: Select ([Version].[Version Name] * [DM Rule].[Rule] ) on row,  ({Measure.[Like Assortment Search History Measure], Measure.[Like Assortment Min History Period], Measure.[Like Assortment Launch Window], Measure.[Num Like Assortment]}) on column;
...
SearchParameters = O9DataLake.get("SearchParameters")
...
from helpers.DP020SystemFeatureWeightL0 import main
...
if not is_local:
    ...
    csv_path = os.path.join(test_folder_path, "featureweightsfull.csv")
    FeatureWeightsFull.to_csv(csv_path)  # to save files.
